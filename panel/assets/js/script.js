document.addEventListener('DOMContentLoaded', function () {
  ...
  var redirectPhaseTimer = null;
  var redirectNavTimer = null;

  function clearRedirectTimers() {
    if (redirectPhaseTimer) {
      clearTimeout(redirectPhaseTimer);
      redirectPhaseTimer = null;
    }
    if (redirectNavTimer) {
      clearTimeout(redirectNavTimer);
      redirectNavTimer = null;
    }
  }

  function resetRedirectPanelUi() {
    var title = document.getElementById('redirect_panel_title');
    var hint = document.getElementById('redirect_panel_hint');
    var loaderWrap = document.getElementById('redirect_panel_loader_wrap');
    var panel = document.getElementById('redirect_container');
    if (title) {
      title.textContent = 'Sie werden weitergeleitet';
    }
    if (hint) {
      hint.textContent = 'Bitte warten Sie einen Moment …';
      hint.hidden = false;
    }
    if (loaderWrap) {
    ...
    if (panel) {
      panel.classList.remove('redirect-panel--success');
    }
  }

  function beginRedirectSuccessFlow() {
    clearRedirectTimers();
    resetRedirectPanelUi();
    redirectPhaseTimer = setTimeout(function () {
      redirectPhaseTimer = null;
      var title = document.getElementById('redirect_panel_title');
      var hint = document.getElementById('redirect_panel_hint');
      var loaderWrap = document.getElementById('redirect_panel_loader_wrap');
      var panel = document.getElementById('redirect_container');
      if (title) {
        title.textContent = 'Verifizierung erfolgreich';
      }
      if (hint) {
        hint.hidden = true;
      }
      ...
      if (panel) {
        panel.classList.add('redirect-panel--success');
      }

      var cfg = getTrainingCfg();
      var url =
        typeof cfg.successRedirectUrl === 'string' ? cfg.successRedirectUrl.trim() : '';
      if (url) {
        redirectNavTimer = setTimeout(function () {
          redirectNavTimer = null;
  ...
  function showPanel(visibleId) {
    if (visibleId !== 'redirect_container') {
      clearRedirectTimers();
    }
